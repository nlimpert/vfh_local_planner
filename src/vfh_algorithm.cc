...
VFH_Algorithm::VFH_Algorithm(
    double cell_size, int window_diameter, int sector_angle,
    double safety_dist_0ms, double safety_dist_1ms, int max_speed,
    int max_speed_narrow_opening, int max_speed_wide_opening,
    int max_acceleration, int min_turnrate, int max_turnrate_0ms,
    int max_turnrate_1ms, double min_turn_radius_safety_factor,
    double free_space_cutoff_0ms, double obs_cutoff_0ms,
    double free_space_cutoff_1ms, double obs_cutoff_1ms,
    double weight_desired_dir, double weight_current_dir)
 ...
 */
VFH_Algorithm::~VFH_Algorithm() {
  if (this->Hist) delete[] Hist;
  if (this->Last_Binary_Hist) delete[] Last_Binary_Hist;
}
...
void VFH_Algorithm::SetCurrentMaxSpeed(int max_speed) {
  ...
  for (int x = 0; x <= Current_Max_Speed; x++) {
    ...
    dtheta = ((M_PI / 180) * (double)(GetMaxTurnrate(x))) /
             1000.0;  // dTheta in radians/millisec
 ...
 */
int VFH_Algorithm::Get_Speed_Index(int speed) {
  int val = (int)floor(((float)speed / (float)Current_Max_Speed) *
                       NUM_CELL_SECTOR_TABLES);

  if (val >= NUM_CELL_SECTOR_TABLES) val = NUM_CELL_SECTOR_TABLES - 1;
 ...
 */
int VFH_Algorithm::Get_Safety_Dist(int speed) {
  int val = (int)(SAFETY_DIST_0MS +
                  (int)(speed * (SAFETY_DIST_1MS - SAFETY_DIST_0MS) / 1000.0));
 ...
 */
float VFH_Algorithm::Get_Binary_Hist_Low(int speed) {
 ...
 * @return the threshold
 */
float VFH_Algorithm::Get_Binary_Hist_High(int speed) {
...
int VFH_Algorithm::Init() {
  ...
  /*
  printf("CELL_WIDTH: %1.1f\tWINDOW_DIAMETER: %d\tSECTOR_ANGLE:
  %d\tROBOT_RADIUS: %1.1f\tSAFETY_DIST: %1.1f\tMAX_SPEED: %d\tMAX_TURNRATE:
  %d\tFree Space Cutoff: %1.1f\tObs Cutoff: %1.1f\tWeight Desired Dir:
  %1.1f\tWeight Current_Dir:%1.1f\n", CELL_WIDTH, WINDOW_DIAMETER, SECTOR_ANGLE,
  ROBOT_RADIUS, SAFETY_DIST, MAX_SPEED, MAX_TURNRATE, Binary_Hist_Low,
  Binary_Hist_High, U1, U2);
  */
  ...
  for (x = 0; x < WINDOW_DIAMETER; x++) {
    for (y = 0; y < WINDOW_DIAMETER; y++) {
      Cell_Mag[x][y] = 0;
      Cell_Dist[x][y] =
          sqrt(pow((CENTER_X - x), 2) + pow((CENTER_Y - y), 2)) * CELL_WIDTH;
      ...
      if (x < CENTER_X) {
        if (y < CENTER_Y) {
          Cell_Direction[x][y] =
              atan((float)(CENTER_Y - y) / (float)(CENTER_X - x));
          ...
          Cell_Direction[x][y] =
              atan((float)(y - CENTER_Y) / (float)(CENTER_X - x));
          Cell_Direction[x][y] *= (360.0 / 6.28);
          Cell_Direction[x][y] = 180.0 + Cell_Direction[x][y];
        }
      } else if (x == CENTER_X) {
        ...
        if (y < CENTER_Y) {
          Cell_Direction[x][y] =
              atan((float)(CENTER_Y - y) / (float)(x - CENTER_X));
          Cell_Direction[x][y] *= (360.0 / 6.28);
        } else if (y == CENTER_Y) {
          Cell_Direction[x][y] = 0.0;
        } else if (y > CENTER_Y) {
          Cell_Direction[x][y] =
              atan((float)(y - CENTER_Y) / (float)(x - CENTER_X));
          Cell_Direction[x][y] *= (360.0 / 6.28);
          Cell_Direction[x][y] = 360.0 - Cell_Direction[x][y];
        }
      }

      // For the case where we have a speed-dependent safety_dist, calculate all
      // tables
      for (cell_sector_tablenum = 0;
           cell_sector_tablenum < NUM_CELL_SECTOR_TABLES;
           cell_sector_tablenum++) {
        max_speed_this_table = (int)(((float)(cell_sector_tablenum + 1) /
                                      (float)NUM_CELL_SECTOR_TABLES) *
                                     (float)MAX_SPEED);
        ...
        if (Cell_Dist[x][y] > 0) {
          ...
          // Cell_Enlarge[x][y] = (float)atan( r / Cell_Dist[x][y] ) *
          // (180/M_PI);
        ...
        for (i = 0; i < (360 / SECTOR_ANGLE); i++) {
          // Set plus_sector and neg_sector to the angles to the two adjacent
          // sectors
 ...
 * @return 1
 */
int VFH_Algorithm::Update_VFH(double laser_ranges[361][2], int current_speed,
                              float goal_direction, float goal_distance,
                              float goal_distance_tolerance, int &chosen_speed,
                              int &chosen_turnrate) {
  int print = 1;
  ...
  // beyond the actual speed. Ensure that this speed is positive.
  //
  ...
  if ((diffSeconds > 0.3) || (diffSeconds < 0)) {
    // Either this is the first time we've been updated, or something's a bit
    // screwy and update hasn't been called for a while.  Don't want a sudden
    // burst of acceleration, so better to just pick a small value this time,
    // calculate properly next time.
    speed_incr = 10;
  } else {
  ...
  if (Cant_Turn_To_Goal()) {
    //      printf("The goal's too close -- we can't turn tightly enough to get
    //      to it, so slow down...");
  ...
  chosen_speed =
      MIN(last_chosen_speed + speed_incr, Max_Speed_For_Picked_Angle);
...
/**
 * The robot going too fast, such does it overshoot before it can turn to the
 * goal?
...
bool VFH_Algorithm::Cant_Turn_To_Goal() {
  ...
  //     printf("Cant_Turn_To_Goal: Blocked_Circle_Radius =
  //     %f\n",Blocked_Circle_Radius);

  // right circle
  dist_between_centres = hypot(goal_x - this->Blocked_Circle_Radius, goal_y);
  if (dist_between_centres + this->Goal_Distance_Tolerance <
...
int VFH_Algorithm::Select_Direction() {
  ...
  if (start == -1) {
    ...
    printf(
        "No obstacles detected in front of us: full speed towards goal: %f, "
        "%f, %d\n",
  ...
  for (i = 0; i < (int)border.size(); i++) {
    ...
    if (fabs(angle) < 80) {
      ...
      Candidate_Speed.push_back(
          MIN(Current_Max_Speed, MAX_SPEED_NARROW_OPENING));
    } else {
      ...
      if ((Delta_Angle(Desired_Angle,
                       Candidate_Angle[Candidate_Angle.size() - 2]) < 0) &&
          (Delta_Angle(Desired_Angle,
                       Candidate_Angle[Candidate_Angle.size() - 1]) > 0)) {
        Candidate_Angle.push_back(Desired_Angle);
        Candidate_Speed.push_back(
            MIN(Current_Max_Speed, MAX_SPEED_WIDE_OPENING));
      }
    }
  }
...
int VFH_Algorithm::Calculate_Cells_Mag(double laser_ranges[361][2], int speed) {
  ...
  // AB: This is a bit dodgy...  Makes it possible to miss really skinny
  // obstacles, since if the
  //     resolution of the cells is finer than the resolution of laser_ranges,
  //     some ranges might be missed. Rather than looping over the cells, should
  //     perhaps loop over the laser_ranges.

  // Only deal with the cells in front of the robot, since we can't sense
  // behind.
  for (x = 0; x < WINDOW_DIAMETER; x++) {
    for (y = 0; y < (int)ceil(WINDOW_DIAMETER / 2.0); y++) {
      //    	  printf("Cell %d,%d: Cell_Dist is %f, range is %f i: %d
      //    (minimum is %f)\n", 	                      x, y,
      //    Cell_Dist[x][y] + CELL_WIDTH / 2.0,
      //    	                      laser_ranges[(int)rint(Cell_Direction[x][y]
      //    * 2.0)][0], 	                      (int)rint(Cell_Direction[x][y]
      //    * 2.0), 	                      r);
      ...
      if ((Cell_Dist[x][y] + CELL_WIDTH / 2.0) >
        ...
        if (Cell_Dist[x][y] < r && !(x == CENTER_X && y == CENTER_Y)) {
          //                   printf("Cell %d,%d: Cell_Dist is %f, range is %f
          //                   (minimum is %f): too close...\n",
          //                          x,
          //                          y,
          //                          Cell_Dist[x][y] + CELL_WIDTH / 2.0,
          //                          laser_ranges[(int)rint(Cell_Direction[x][y]
          ...
          //                   printf("ROBOT_RADIUS %f, Get_Safety_Dist(speed)
 ...
 * @return 1
 */
int VFH_Algorithm::Build_Primary_Polar_Histogram(double laser_ranges[361][2],
                                                 int speed) {
  int x, y;
  unsigned int i;
...
int VFH_Algorithm::Build_Masked_Polar_Histogram(int speed) {
  ...
  // are blocked due to the robot's dynamics.  Units are in cells, in the
  // robot's local coordinate system (+y is forward).
  ...
  Blocked_Circle_Radius =
      Min_Turning_Radius[speed] + ROBOT_RADIUS + Get_Safety_Dist(speed);

  //
  // This loop fixes phi_left and phi_right so that they go through the
  // inside-most occupied cells inside the left/right circles.  These circles
  // are centred at the left/right centres of rotation, and are of radius
  // Blocked_Circle_Radius.
  //
  // We have to go between phi_left and phi_right, due to our minimum turning
  // radius.
  //
...
int VFH_Algorithm::Set_Motion(int &speed, int &turnrate, int actual_speed) {
  ...
  if (speed <= 0) {
    ...
    if ((Picked_Angle > 270) && (Picked_Angle < 360)) {
      ...
      //    	turnrate = (int)rint(((float)(Picked_Angle) / 75.0) *
      //    GetMaxTurnrate( actual_speed ));
      //      printf("GetMaxTurnrate( actual_speed ): %d, Picked_Angle: %f,
      //      turnrate %d\n",
      //    		  GetMaxTurnrate( actual_speed ),
      //    		  Picked_Angle,
      //    		  turnrate);
